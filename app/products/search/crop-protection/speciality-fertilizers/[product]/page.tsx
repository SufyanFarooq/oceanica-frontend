...
export default async function ProductDetail({ params }: { params: Promise<{ product: string }> }) {
  ...
  const product = getProductBySlug('specialityFertilizers', resolvedParams.product)
  if (!product) {
    // NOTE: notFound() is server-only in App Router; if you hit issues, replace with a client redirect.
    notFound()
  }

  const relatedProducts = getRelatedProducts('specialityFertilizers', product.id, true).slice(0, 3)
      ...
      <section className="bg-white/80 py-4">
        <div className="mx-auto max-w-7xl px-4">
          <nav className="flex items-center gap-2 text-sm text-slate-600">
            ...
            <span>/</span>
            <Link href="/products/search/crop-protection/speciality-fertilizers" className="hover:text-emerald-700">
          ...
          <div className="lg:col-span-7">
            <div className="lg:sticky lg:top-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-600">
                ...
                <span className="rounded-full bg-white px-3 py-1">{locale === 'en' ? 'High Efficiency • Water Soluble' : 'اعلیٰ کارکردگی • پانی میں حل پذیر'}</span>
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-emerald-900 via-emerald-800 to-emerald-900 p-10">
            ...
            <div className="relative">
              <h2 className="text-3xl font-bold text-white md:text-4xl">
                {locale === 'en' ? 'Ready for Premium Results?' : 'اعلیٰ نتائج کے لیے تیار ہیں؟'}
              </h2>
              <Link
                href="/contact-us"
